...
SECTIONS = [
    {
        ...
        "body": """
        <p><strong>Core idea:</strong> The CLT tells us that X̄ ≈ N(μ, σ²/n). We can use this to construct confidence intervals for μ.</p>

        <p><strong>When σ is known:</strong> A (1 - α)·100% confidence interval is:</p>
        <pre class='code-block'>X̄ ± z_{α/2} · (σ / √n)</pre>

        <p>where z_{α/2} is the critical value such that P(Z ≤ z_{α/2}) = 1 - α/2 for standard normal Z.</p>

        <p><strong>When σ is unknown:</strong> Replace σ with the sample standard deviation S and use t_{α/2, n-1}:</p>
        <pre class='code-block'>X̄ ± t_{α/2, n-1} · (S / √n)</pre>

        <p>The t-distribution accounts for the extra uncertainty from estimating σ. It has heavier tails than normal, giving wider intervals.</p>

        <div class='worked-example'>
        <p><strong>Example: CI with known σ</strong></p>
        <pre class='code-block'>Data: n = 100 test scores, X̄ = 75.2, σ = 8 (from historical data)
...
CI = 75.2 ± 1.96 · (8 / √100)
   ...
   = (73.632, 76.768)</pre>
        ...
        <ul>
        <li><strong>Margin of error (ME):</strong> E = z_{α/2} · σ / √n (half-width of the interval)</li>
        <li><strong>Standard error (SE):</strong> σ / √n (denominator of ME)</li>
        <li><strong>Confidence level:</strong> 1 - α (e.g., 0.95 for 95% CI, meaning α = 0.05)</li>
        </ul>
        """
    },
    {
        ...
        <p><strong>Formula:</strong> To ensure margin of error ≤ E with confidence 1 - α:</p>
...
From pilot study, estimate σ = 25 (satisfaction score out of 100)
z_{0.025} = 1.96
...
Need at least 267 patients surveyed.</pre>
        ...
        <ul>
        <li><strong>Smaller margin of error (E) → larger n:</strong> E and n are inversely related. To halve E requires 4× the sample size.</li>
        <li><strong>Higher confidence (larger 1 - α) → larger n:</strong> 99% CI (z = 2.576) requires more data than 95% CI (z = 1.96).</li>
        <li><strong>Larger σ → larger n:</strong> More variable populations require larger samples.</li>
        </ul>

        <div class='success-box'>
        <p><strong>Rule of thumb:</strong> To halve the margin of error, quadruple the sample size (since ME ∝ 1/√n).</p>
        </div>
        """
    },
    {
...
Interpretation: 95% confident true support is between 48.9% and 55.1% (±3.1% margin)</pre>
        ...
        <li><strong>Small samples:</strong> Rule of thumb n ≥ 30, but depends on data distribution.</li>
