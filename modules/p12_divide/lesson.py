"""Division Basics — Lesson sections."""

SECTIONS = [
    {
        "title": "What is Division? Sharing Equally 🍪",
        "body": """<p>Division means <strong>sharing something equally</strong> among people or groups. It's about making fair shares!</p>
<p>Imagine you have 12 cookies and 3 friends. If you want to share the cookies so everyone gets the same amount, you divide!</p>
<svg viewBox="0 0 430 200" style="width:100%;max-width:430px;height:auto;display:block;margin:16px auto;">
  <text x="215" y="25" text-anchor="middle" fill="currentColor" font-size="14" font-weight="bold">12 cookies shared among 3 friends:</text>

  <!-- Friend 1: 4 cookies -->
  <circle cx="55" cy="80" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="85" cy="80" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="55" cy="110" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="85" cy="110" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <rect x="30" y="55" width="80" height="80" rx="4" fill="none" stroke="#4f8ef7" stroke-width="2"/>
  <text x="70" y="160" text-anchor="middle" fill="currentColor" font-size="12" font-weight="bold">Friend 1</text>

  <!-- Friend 2: 4 cookies -->
  <circle cx="195" cy="80" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="225" cy="80" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="195" cy="110" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="225" cy="110" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <rect x="170" y="55" width="80" height="80" rx="4" fill="none" stroke="#4f8ef7" stroke-width="2"/>
  <text x="210" y="160" text-anchor="middle" fill="currentColor" font-size="12" font-weight="bold">Friend 2</text>

  <!-- Friend 3: 4 cookies -->
  <circle cx="335" cy="80" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="365" cy="80" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="335" cy="110" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <circle cx="365" cy="110" r="13" fill="#f59e0b" stroke="#d97706" stroke-width="1"/>
  <rect x="310" y="55" width="80" height="80" rx="4" fill="none" stroke="#4f8ef7" stroke-width="2"/>
  <text x="350" y="160" text-anchor="middle" fill="currentColor" font-size="12" font-weight="bold">Friend 3</text>

  <text x="215" y="190" text-anchor="middle" fill="currentColor" font-size="14">Each friend gets 4 cookies!</text>
</svg>
<div class='example-box'>
<strong>Division symbol: ÷</strong><br>
<br>
12 ÷ 3 = 4<br>
<br>
This means: 12 things shared among 3 people = 4 things each
</div>
<p><strong>Why learn division?</strong> Division helps us share fairly, split costs, and organize things into equal groups. It's a skill we use every day!</p>"""
    },
    {
        "title": "Division by Grouping: Making Equal Groups 📦",
        "body": """<p>Another way to think about division is by <strong>grouping</strong>. We take a total amount and put it into equal-sized groups.</p>
<p><strong>Example: 12 ÷ 3</strong> can mean "How many groups of 3 can we make from 12?"</p>
<svg viewBox="0 0 430 200" style="width:100%;max-width:430px;height:auto;display:block;margin:16px auto;">
  <text x="215" y="25" text-anchor="middle" fill="currentColor" font-size="14" font-weight="bold">12 apples in groups of 3:</text>

  <!-- Group 1 -->
  <rect x="15" y="45" width="90" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="35" cy="70" r="8" fill="#ef4444"/>
  <circle cx="60" cy="70" r="8" fill="#ef4444"/>
  <circle cx="85" cy="70" r="8" fill="#ef4444"/>
  <text x="60" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 1</text>

  <!-- Group 2 -->
  <rect x="120" y="45" width="90" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="140" cy="70" r="8" fill="#ef4444"/>
  <circle cx="165" cy="70" r="8" fill="#ef4444"/>
  <circle cx="190" cy="70" r="8" fill="#ef4444"/>
  <text x="165" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 2</text>

  <!-- Group 3 -->
  <rect x="225" y="45" width="90" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="245" cy="70" r="8" fill="#ef4444"/>
  <circle cx="270" cy="70" r="8" fill="#ef4444"/>
  <circle cx="295" cy="70" r="8" fill="#ef4444"/>
  <text x="270" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 3</text>

  <!-- Group 4 -->
  <rect x="330" y="45" width="90" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="350" cy="70" r="8" fill="#ef4444"/>
  <circle cx="375" cy="70" r="8" fill="#ef4444"/>
  <circle cx="400" cy="70" r="8" fill="#ef4444"/>
  <text x="375" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 4</text>

  <text x="215" y="145" text-anchor="middle" fill="#1a1a2e" font-size="14" font-weight="bold">We made 4 groups of 3!</text>
  <text x="215" y="170" text-anchor="middle" fill="#1a1a2e" font-size="14">So 12 ÷ 3 = 4 groups</text>
</svg>
<div class='example-box'>
<strong>Two ways to think about 12 ÷ 3:</strong><br>
<br>
1. <strong>Sharing:</strong> Divide 12 equally among 3 people → each gets 4<br>
2. <strong>Grouping:</strong> Make groups of 3 from 12 → we get 4 groups<br>
<br>
Both give the same answer: 4!
</div>
<p><strong>Key insight:</strong> Division and multiplication are opposites. 3 × 4 = 12, so 12 ÷ 3 = 4!</p>"""
    },
    {
        "title": "Division as the Opposite of Multiplication 🔄",
        "body": """<p>Multiplication and division are <strong>inverse operations</strong>. They undo each other!</p>
<p>If you know a multiplication fact, you automatically know a division fact.</p>
<svg viewBox="0 0 430 210" style="width:100%;max-width:430px;height:auto;display:block;margin:16px auto;">
  <text x="215" y="25" text-anchor="middle" fill="currentColor" font-size="15" font-weight="bold">Multiplication and Division are Opposites</text>

  <!-- Multiplication box -->
  <rect x="25" y="50" width="170" height="85" rx="4" fill="#dbeafe" stroke="#4f8ef7" stroke-width="2"/>
  <text x="110" y="75" text-anchor="middle" fill="#1a1a2e" font-size="14" font-weight="bold">Multiplication</text>
  <text x="110" y="100" text-anchor="middle" fill="#1a1a2e" font-size="14">3 × 4 = 12</text>
  <text x="110" y="120" text-anchor="middle" fill="#1a1a2e" font-size="12">"3 groups of 4"</text>

  <!-- Arrow -->
  <text x="215" y="100" text-anchor="middle" fill="#1a1a2e" font-size="22" font-weight="bold">↔</text>

  <!-- Division box -->
  <rect x="235" y="50" width="170" height="85" rx="4" fill="#dcfce7" stroke="#22c55e" stroke-width="2"/>
  <text x="320" y="75" text-anchor="middle" fill="#1a1a2e" font-size="14" font-weight="bold">Division</text>
  <text x="320" y="100" text-anchor="middle" fill="#1a1a2e" font-size="14">12 ÷ 3 = 4</text>
  <text x="320" y="120" text-anchor="middle" fill="#1a1a2e" font-size="12">"12 split into 3"</text>

  <text x="215" y="165" text-anchor="middle" fill="#1a1a2e" font-size="13">If 3 × 4 = 12, then 12 ÷ 3 = 4 and 12 ÷ 4 = 3</text>
  <text x="215" y="190" text-anchor="middle" fill="#1a1a2e" font-size="13">If you know your times tables, you know division!</text>
</svg>
<div class='example-box'>
<strong>Quick facts:</strong><br>
2 × 5 = 10, so 10 ÷ 2 = 5 and 10 ÷ 5 = 2<br>
6 × 7 = 42, so 42 ÷ 6 = 7 and 42 ÷ 7 = 6<br>
8 × 3 = 24, so 24 ÷ 8 = 3 and 24 ÷ 3 = 8
</div>
<p><strong>Why is this helpful?</strong> When you're learning division, use multiplication to check your answer! If 20 ÷ 4 = 5, check: 5 × 4 = 20 ✓</p>"""
    },
    {
        "title": "Understanding Remainders: What's Left Over? 🤔",
        "body": """<p>Sometimes when we divide, we can't split things into equal groups perfectly. We have something left over called a <strong>remainder</strong>.</p>
<p><strong>Example: 13 ÷ 4</strong></p>
<svg viewBox="0 0 430 230" style="width:100%;max-width:430px;height:auto;display:block;margin:16px auto;">
  <text x="215" y="25" text-anchor="middle" fill="currentColor" font-size="14" font-weight="bold">13 apples in groups of 4:</text>

  <!-- Group 1 -->
  <rect x="15" y="45" width="80" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="35" cy="60" r="7" fill="#ef4444"/>
  <circle cx="55" cy="60" r="7" fill="#ef4444"/>
  <circle cx="75" cy="60" r="7" fill="#ef4444"/>
  <circle cx="55" cy="85" r="7" fill="#ef4444"/>
  <text x="55" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 1</text>

  <!-- Group 2 -->
  <rect x="115" y="45" width="80" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="135" cy="60" r="7" fill="#ef4444"/>
  <circle cx="155" cy="60" r="7" fill="#ef4444"/>
  <circle cx="175" cy="60" r="7" fill="#ef4444"/>
  <circle cx="155" cy="85" r="7" fill="#ef4444"/>
  <text x="155" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 2</text>

  <!-- Group 3 -->
  <rect x="215" y="45" width="80" height="70" rx="4" fill="#fef9c3" stroke="#f59e0b" stroke-width="2"/>
  <circle cx="235" cy="60" r="7" fill="#ef4444"/>
  <circle cx="255" cy="60" r="7" fill="#ef4444"/>
  <circle cx="275" cy="60" r="7" fill="#ef4444"/>
  <circle cx="255" cy="85" r="7" fill="#ef4444"/>
  <text x="255" y="100" text-anchor="middle" fill="#1a1a2e" font-size="11" font-weight="bold">Group 3</text>

  <!-- Remainder -->
  <rect x="340" y="55" width="50" height="50" rx="4" fill="#fecaca" stroke="#ef4444" stroke-width="2" stroke-dasharray="5"/>
  <circle cx="365" cy="80" r="7" fill="#ef4444"/>
  <text x="365" y="130" text-anchor="middle" fill="#ef4444" font-size="11" font-weight="bold">Left over!</text>

  <text x="215" y="175" text-anchor="middle" fill="#1a1a2e" font-size="14" font-weight="bold">13 ÷ 4 = 3 remainder 1 (or 3 R1)</text>
  <text x="215" y="200" text-anchor="middle" fill="#1a1a2e" font-size="13">Check: (3 × 4) + 1 = 12 + 1 = 13 ✓</text>
</svg>
<div class='example-box'>
<strong>Reading the answer:</strong><br>
<br>
13 ÷ 4 = 3 R1<br>
<br>
This means: 13 divided into groups of 4 gives us 3 complete groups with 1 left over
</div>
<p><strong>How to check:</strong> (3 × 4) + 1 = 12 + 1 = 13 ✓</p>
<p><strong>Real-world example:</strong> If you have 13 cookies and want to give 4 to each friend, you can give 4 to each of 3 friends. You'll have 1 cookie left!</p>"""
    },
    {
        "title": "Division Facts & Strategies for Success 🎯",
        "body": """<p>To get really good at division, it helps to know <strong>division facts</strong> — just like your multiplication times tables!</p>
<p>Here are some important division facts to remember:</p>
<svg viewBox="0 0 430 280" style="width:100%;max-width:430px;height:auto;display:block;margin:16px auto;">
  <text x="215" y="25" text-anchor="middle" fill="currentColor" font-size="14" font-weight="bold">Division Facts (by 2 and by 5)</text>

  <!-- Divide by 2 box -->
  <rect x="15" y="45" width="190" height="130" rx="4" fill="#dbeafe" stroke="#4f8ef7" stroke-width="2"/>
  <text x="110" y="70" text-anchor="middle" fill="#1a1a2e" font-size="13" font-weight="bold">Divide by 2:</text>
  <text x="110" y="92" text-anchor="middle" fill="#1a1a2e" font-size="12">4 ÷ 2 = 2</text>
  <text x="110" y="112" text-anchor="middle" fill="#1a1a2e" font-size="12">6 ÷ 2 = 3</text>
  <text x="110" y="132" text-anchor="middle" fill="#1a1a2e" font-size="12">8 ÷ 2 = 4</text>
  <text x="110" y="152" text-anchor="middle" fill="#1a1a2e" font-size="12">10 ÷ 2 = 5</text>
  <text x="110" y="172" text-anchor="middle" fill="#1a1a2e" font-size="12">12 ÷ 2 = 6</text>

  <!-- Divide by 5 box -->
  <rect x="225" y="45" width="190" height="130" rx="4" fill="#dcfce7" stroke="#22c55e" stroke-width="2"/>
  <text x="320" y="70" text-anchor="middle" fill="#1a1a2e" font-size="13" font-weight="bold">Divide by 5:</text>
  <text x="320" y="92" text-anchor="middle" fill="#1a1a2e" font-size="12">5 ÷ 5 = 1</text>
  <text x="320" y="112" text-anchor="middle" fill="#1a1a2e" font-size="12">10 ÷ 5 = 2</text>
  <text x="320" y="132" text-anchor="middle" fill="#1a1a2e" font-size="12">15 ÷ 5 = 3</text>
  <text x="320" y="152" text-anchor="middle" fill="#1a1a2e" font-size="12">20 ÷ 5 = 4</text>
  <text x="320" y="172" text-anchor="middle" fill="#1a1a2e" font-size="12">25 ÷ 5 = 5</text>

  <text x="215" y="210" text-anchor="middle" fill="#1a1a2e" font-size="13" font-weight="bold">Helpful Strategies:</text>
  <text x="215" y="232" text-anchor="middle" fill="#1a1a2e" font-size="12">1. Use multiplication facts! If 6 × 3 = 18, then 18 ÷ 6 = 3</text>
  <text x="215" y="252" text-anchor="middle" fill="#1a1a2e" font-size="12">2. Draw pictures or use objects to show the division</text>
  <text x="215" y="272" text-anchor="middle" fill="#1a1a2e" font-size="12">3. Count in groups: For 12 ÷ 3, count "3, 6, 9, 12" = 4 groups!</text>
</svg>
<p><strong>Practice tip:</strong> The more division facts you memorize, the faster and easier division becomes!</p>"""
    }
]
